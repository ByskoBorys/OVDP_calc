...
- Для **купонних** паперів → **формула Мінфіну**:  
Для коректного розрахунку від **дати аукціону** використовуйте:
  - для **гривневих ОВДП** — **дату розрахунку +1 день**,
  - для **доларових ОВДП** — **дату розрахунку +2 дні**.
...
2. Ми рахуємо **dirty-ціну** на обидві дати (SIM або YTM — залежно від паперу на кожну із дат).
3. Додаємо **усі купони**, що припали між датами володіння (у день погашення — окремо купон і номінал).
4. **P&L (сума)** = ціна продажу − ціна купівлі + отримані купони.  
   **Річна проста дохідність** = (P&L / ціна купівлі) × (365 / днів у позиції) × 100%.
...
### Важливо
- Розрахунки базуються на довіднику НБУ; можливі відмінності від біржових котирувань через округлення.  
- Це **не інвестиційна порада**. Перевіряйте параметри паперу та розрахунки перед прийняттям рішень.
"""
